<%@ Page Title="Productos" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="listar_tbl_producto.aspx.cs" Inherits="Monolito_4am_DB.Mantenimiento.listar_tbl_producto" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="card card-custom">
        <div class="card-body p-4">
            ...
            <!-- Titulo y Boton Superior -->
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h4 class="m-0 font-weight-bold text-dark" style="font-size: 1.5rem;">
                    <i class="fa-solid fa-box text-warning mr-2"></i> Gesti&oacute;n de Productos
                </h4>
                <a href="nuevo_tbl_producto.aspx" class="btn text-white px-4 font-weight-bold shadow-sm" style="background-color: #2563eb; border-radius: 50rem;">
                    + Nuevo Producto
                </a>
            </div>

            <!-- Caja de Filtros (FUERA del UpdatePanel para mantener el foco del cursor al escribir) -->
            <div class="filter-box mb-4">
                <div class="row align-items-end">
                    <div class="col-md-3">
                        <label class="font-weight-bold text-dark mb-2">Buscar por:</label>
                        <asp:DropDownList ID="ddlFiltro" runat="server" CssClass="form-control form-control-pill" style="border-color: #2563eb;">
                    ...
                    <div class="col-md-7">
                        <label class="font-weight-bold text-dark mb-2">T&eacute;rmino de b&uacute;squeda:</label>
                        <asp:TextBox ID="txtBuscar" runat="server" CssClass="form-control form-control-pill" Placeholder="Escribe para buscar..." autocomplete="off"></asp:TextBox>
                    </div>
                    <div class="col-md-2">
                ...
                <ContentTemplate>
                    ...
                    <div class="table-responsive">
                        <asp:GridView ID="gvProductos" runat="server" AutoGenerateColumns="False" 
                            ...
                            DataKeyNames="pro_id" OnRowCommand="gvProductos_RowCommand"
                            ...
                            <Columns>
                                <asp:BoundField DataField="pro_id" HeaderText="ID" SortExpression="pro_id" />
                                <asp:TemplateField HeaderText="Imagen">
                                    <ItemTemplate>
                                        <img src='<%# Eval("primera_imagen") %>' alt="Producto" style="width: 50px; height: 50px; object-fit: cover; border-radius: 8px; border: 1px solid #e2e8f0;" />
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:BoundField DataField="pro_nombre" HeaderText="Nombre" SortExpression="pro_nombre" />
                                <asp:BoundField DataField="pro_cantidad" HeaderText="Cantidad" SortExpression="pro_cantidad" />
                                <asp:BoundField DataField="pro_precio" HeaderText="Precio" DataFormatString="{0:C}" SortExpression="pro_precio" />
                                <asp:TemplateField HeaderText="Proveedor" SortExpression="prov_nombre">
                                    <ItemTemplate>
                                        <%# Eval("prov_nombre").ToString() == "Sin proveedor" 
                                            ? "<span style='color: #94a3b8; font-style: italic;'><i class='fa-solid fa-triangle-exclamation mr-1'></i>Sin proveedor</span>" 
                                            : Eval("prov_nombre") %>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                
                                <asp:TemplateField HeaderText="Estado" SortExpression="pro_estado">
                                    <ItemTemplate>
                                        <span class='<%# Eval("pro_estado").ToString() == "A" ? "badge-activo" : "badge-inactivo" %>'>
                                            <%# Eval("pro_estado").ToString() == "A" ? "Activo" : "Inactivo" %>
                                        </span>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                ...
                                <asp:TemplateField HeaderText="Acciones">
                                    <ItemTemplate>
                                        <asp:LinkButton ID="btnEditar" runat="server" CommandName="Editar" CommandArgument='<%# Eval("pro_id") %>' CssClass="btn-action btn-edit mr-2" ToolTip="Editar producto">
                                            <i class="fa-solid fa-pencil mr-1"></i> Editar
                                        </asp:LinkButton>
                                        
                                        <asp:LinkButton ID="btnDesactivar" runat="server" CommandName="Desactivar" CommandArgument='<%# Eval("pro_id") %>' 
                                            CssClass="btn-action btn-deactivate btn-swal-confirm mr-2" ToolTip="Desactivar producto"
                                            Visible='<%# Eval("pro_estado").ToString() == "A" %>'
                                            data-swal-title="&iquest;Desactivar producto?" data-swal-text="El producto no estar&aacute; disponible para ventas.">
                                            <i class="fa-solid fa-ban mr-1"></i> Desactivar
                                        </asp:LinkButton>
                                        
                                        <asp:LinkButton ID="btnActivar" runat="server" CommandName="Activar" CommandArgument='<%# Eval("pro_id") %>' 
                                            CssClass="btn-action btn-activate btn-swal-confirm mr-2" ToolTip="Activar producto"
                                            Visible='<%# Eval("pro_estado").ToString() == "I" %>'
                                            data-swal-title="&iquest;Activar producto?" data-swal-text="El producto volver&aacute; a estar disponible.">
                                        ...
                                        <asp:LinkButton ID="btnEliminar" runat="server" CommandName="Eliminar" CommandArgument='<%# Eval("pro_id") %>' 
                                            ...
                                            data-swal-title="&iquest;Eliminar permanentemente?" data-swal-text="&iexcl;Esta acci&oacute;n eliminar&aacute; el producto de la base de datos!" data-swal-type="warning">
                            ...
                            <EmptyDataTemplate>
                                <div class="text-center py-5">
                                    ...
                                    <h5 class="text-muted font-weight-bold">No se encontraron productos</h5>
                                </div>
                            </EmptyDataTemplate>
    ...
    <script>
        (function () {
            ...
            function setupLiveFilter() {
                ...
                var ddl = document.getElementById('<%= ddlFiltro.ClientID %>');
            ...
            document.addEventListener('DOMContentLoaded', function () {
                setupLiveFilter();
                // Re-configurar despu\u00e9s de cada postback AJAX (UpdatePanel)
                if (typeof Sys !== 'undefined' && Sys.WebForms) {
                    Sys.WebForms.PageRequestManager.getInstance().add_endRequest(setupLiveFilter);
                }
            });

            // SweetAlert2 con delegaci\u00f3n de eventos (funciona con UpdatePanel y paginaci\u00f3n)
